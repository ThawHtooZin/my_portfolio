        ...
        <AnimatePresence>
          {isMenuOpen && (
            <motion.div
              ...
              className="md:hidden fixed left-0 right-0 top-16 px-2 pt-2 pb-3 space-y-3 sm:px-3 bg-black bg-opacity-90 backdrop-blur-xl rounded-b-2xl shadow-2xl border-b border-purple-700 z-40 w-full max-w-full overflow-hidden"
              style={{ 
                boxShadow: '0 8px 32px 0 rgba(88, 28, 135, 0.25)',
                minHeight: '200px',
                maxWidth: '100vw',
                left: '0',
                right: '0'
            ...
            >
              {navItems.map((item, index) => (
                <motion.a
                  ...
                  className={`relative text-white hover:text-transparent bg-gradient-to-r from-purple-400 to-blue-500 bg-clip-text block px-3 py-4 rounded-md text-lg font-semibold transition-all duration-300 hover:drop-shadow-[0_0_8px_rgba(168,85,247,0.6)] hover:drop-shadow-[0_0_15px_rgba(59,130,246,0.4)] group hover:scale-95 focus:outline-none focus:ring-2 focus:ring-purple-400 focus:ring-opacity-50 w-full ${isMenuOpen ? 'translate-x-0' : '-translate-x-4'}`}
                  initial={{ opacity: 0, x: -20 }}
                  animate={{ opacity: 1, x: 0 }}
                  transition={{ duration: 0.3, delay: index * 0.1 }}
                >
                  <span className="absolute inset-0 bg-gradient-to-r from-purple-400 to-blue-500 opacity-0 group-hover:opacity-20 rounded-md transition-opacity duration-300"></span>
              ...
              <motion.div 
                className="border-t border-purple-700 pt-3 mt-3 w-full"
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
                transition={{ duration: 0.3, delay: 0.4 }}
              >
                <p className="text-white text-base font-medium block px-3 py-2 w-full">
                  Member of <a href="https://protechmm.com/" className="relative inline-block text-yellow-400 hover:text-white hover:drop-shadow-[0_0_10px_rgba(255,255,255,0.8)] transition-all duration-300 group focus:outline-none focus:ring-2 focus:ring-yellow-400 focus:ring-opacity-50">
                    <span className="absolute bottom-0 left-0 w-0 h-0.5 bg-white transition-all duration-300 group-hover:w-full"></span>
                    ProTechMM
                    <span className="absolute top-0 -right-1 translate-x-1/2 -translate-y-1/2 text-lg rotate-45">👑</span>
                  </a>
